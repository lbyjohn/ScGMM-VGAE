# The scGMM-VGAE model is developed based on the GMM-VGAE model (Hui et al., 2020)
# Following codes are the GMM and VGAE modules within the scGMM-VGAE algorithm
# These models are used for clustering unlabeled data
# The codes are developed and modified based on the codes for labeled data to perform on unlabeled data
...
class GMM_VGAE(nn.Module):
    ...
    def train(self, acc_list, adj_norm, features, adj_label, y, weight_tensor, norm, optimizer, epochs, lr, save_path, dataset, features_new):
        ...
        logwriter = csv.DictWriter(logfile, fieldnames=['iter', 'acc', 'nmi', 'ari', 'silhouette', 'f1_macro', 'f1_micro', 'precision_macro', 'precision_micro', 'Loss_recons', 'Loss_clus' , 'Loss_elbo'])
        ...
        for epoch in epoch_bar:
            ...
            #Save logs 
            logdict = dict(iter = epoch, acc = acc, nmi= nmi, ari=adjscore, silhouette=sc, f1_macro=f1_macro , f1_micro=f1_micro, precision_macro=precision_macro, precision_micro = precision_micro, Loss_recons=Loss_recons.detach().cpu().numpy(), Loss_clus=Loss_clus.detach().cpu().numpy(), Loss_elbo=Loss_elbo.detach().cpu().numpy())
